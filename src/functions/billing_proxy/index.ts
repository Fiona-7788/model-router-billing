 ...
 * 表单 UUID: FORM_4305FA38D1C64C2EB9D45704C314F490 (账单归档V2，通过平台UI创建，数据表已初始化)
 */
  ...
  try {
    ...
    if (ctx?.form && typeof ctx.form.queryMany === "function") {
      try {
        ...
        queryAttempts.push(`form.queryMany:${e?.message?.slice(0,50) || 'err'}`);
    ...
    const matchedItems = Array.isArray(items) ? items.filter((item: any) => {
      ...
      // 直接字符串匹配
      if (typeof archiveDate === "string" && archiveDate.startsWith(targetDate)) return true;
